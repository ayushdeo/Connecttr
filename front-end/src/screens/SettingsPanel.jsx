    ...
    return (
        <div className="flex flex-col p-8 min-h-screen text-mist font-sans w-full max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="mb-8 text-center sm:text-left">
                <h1 className="text-3xl font-bold text-mist mb-2">Settings</h1>
...
const SettingsRow = ({ icon, label, subLabel, badge, danger, onClick }) => (
    <button
        onClick={onClick}
        className={`w-full flex items-center justify-between p-4 transition-colors border-b border-overlay/5 last:border-0 hover:bg-lilac-mist/5 group`}>
        <div className="flex items-center gap-4">
            <div className={`p-2 rounded-xl transition-colors ${danger
                ? 'bg-rose-500/10 text-rose-400 group-hover:bg-rose-500/20'
                : 'bg-overlay/5 text-soft-violet group-hover:bg-royal-amethyst/20 group-hover:text-royal-amethyst'
                }`}>
                {icon}
            </div>
            <div className="text-left">
                <p className={`font-medium ${danger ? 'text-rose-400' : 'text-mist'}`}>{label}</p>
        ...
        <div className="flex items-center gap-3">
            ...
            <ChevronRight size={16} className="text-overlay/20 group-hover:text-overlay/50 transition-colors" />
        </div>
    </button>
);
