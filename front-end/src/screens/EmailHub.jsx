      ...
      `}>
        ...
        <div className={`flex items-end gap-3 mb-2 ${isOutbound ? "flex-row-reverse" : "flex-row"}`}>
          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shadow-lg
            ${isOutbound ? "bg-royal-amethyst text-white" : "bg-slate border border-overlay/10 text-soft-violet"}`}>
            {isOutbound ? "You" : senderName.charAt(0)}
          </div>
          <div className={`flex flex-col text-xs ${isOutbound ? "items-end" : "items-start"}`}>
            <span className="font-bold text-mist">{isOutbound ? "You" : senderName}</span>
        ...
        <div className={`rounded-2xl p-5 shadow-xl border backdrop-blur-sm relative overflow-hidden
          ${isOutbound
            ...
            : 'bg-slate/60 border-overlay/5 text-mist rounded-tl-sm'
          }
        `}>
          ...
          {/* Quoted Section */}
          {quoted && (
            <div className="mt-4 pt-3 border-t border-overlay/5">
              {!expanded ? (
                <button
                ...
                <div className="animate-in fade-in slide-in-from-top-2 duration-300">
                  <div className="pl-3 border-l-2 border-overlay/10 text-xs text-soft-violet/60 whitespace-pre-wrap font-mono leading-tight">
                    {quoted}
                  </div>
                  <button
        ...
        {message.events && message.events.length > 0 && (
          <div className={`mt-3 flex flex-wrap gap-2 ${isOutbound ? "justify-end" : "justify-start"}`}>
            {message.events.map((ev, i) => (
              <div key={i} className="flex items-center gap-1.5 px-2 py-1 rounded-full bg-overlay/5 border border-overlay/5 text-[10px] text-soft-violet/80 shadow-sm">
                ...
                {ev.CreatedAt && <span className="opacity-50 border-l border-overlay/10 pl-1.5 ml-0.5">
...
const EmailHub = () => {
  ...
  const [filter, setFilter] = useState("Has Email");
  ...
  const filteredLeads = leads.filter(l => {
    ...
    if (filter === "Has Email") return !!l.email && l.status !== "Needs Email";
    if (filter === "No Email") return !l.email || l.status === "Needs Email";
    if (filter === "Replied") return l.status === "Responded";
    return true; // "All"
  });
  ...
  return (
    <div className="flex h-[calc(100vh-64px)] overflow-hidden bg-ink text-mist font-sans">
      ...
      {/* COLUMN 1: LEADS LIST */}
      <div className="theme-surface w-80 flex flex-col border-r border-overlay/5 bg-slate/30 backdrop-blur-md">
        <div className="p-4 border-b border-overlay/5 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="font-bold text-mist flex items-center gap-2">
          ...
          <div className="relative group">
            ...
            <input
              className="w-full bg-midnight-plum/50 border border-overlay/5 rounded-lg pl-9 pr-3 py-2 text-sm text-mist focus:outline-none focus:border-royal-amethyst/50 transition-all placeholder:text-mist/20"
          ...
          <div className="flex gap-1 p-1 bg-midnight-plum/30 rounded-lg border border-overlay/5">
            {["Has Email", "All", "Replied", "No Email"].map(f => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`flex-1 text-[10px] py-1.5 rounded-md transition-all font-medium ${filter === f ? "bg-royal-amethyst text-white shadow" : "text-soft-violet hover:text-mist hover:bg-overlay/5"}`}
              >
                {f}
              </button>
            ))}
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          {loadingLeads ? (
            ...
            <div className="p-8 text-center text-mist/30 italic text-sm">No conversations found</div>
          ) : (
            filteredLeads.map(lead => (
              <div
                key={lead.id}
                onClick={() => setSelectedLead(lead)}
                className={`p-4 border-b border-overlay/5 cursor-pointer transition-all hover:bg-overlay/5 group relative
                  ${selectedLead?.id === lead.id ? "bg-overlay/5 border-l-2 border-l-royal-amethyst" : "border-l-2 border-l-transparent"}
                `}
              >
                  ...
                  <div className={`text-[10px] px-2 py-0.5 rounded-full font-medium inline-flex items-center gap-1
                    ${lead.status === "Responded"
                      ...
                      : lead.status === "Opened"
                        ...
                        : "bg-overlay/5 text-slate-400 border border-overlay/5"
                    }`}>
      ...
      <div className="theme-surface flex-1 flex flex-col bg-midnight-plum/40 relative">
        {selectedLead ? (
          <>
            {/* Thread Header */}
            <div className="h-16 border-b border-overlay/5 flex items-center px-6 justify-between bg-ink/50 backdrop-blur-sm z-10 sticky top-0">
              <div>
                <h3 className="font-bold text-lg text-mist flex items-center gap-2">
                  {selectedLead.name}
                ...
                <div className="flex items-center gap-3 text-xs text-soft-violet mt-0.5">
                  ...
                  <span className="w-1 h-1 rounded-full bg-overlay/10"></span>
            ...
            {/* Messages Area */}
            <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-2 scroll-smooth">
              {loadingThread ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-soft-violet/30">
            <div className="w-24 h-24 rounded-full bg-overlay/5 flex items-center justify-center mb-6 animate-pulse">
              <Inbox size={48} className="opacity-50" />
            </div>
            <p className="text-lg font-medium text-mist/50">Select a conversation</p>
      ...
      {/* COLUMN 3: COMPOSER */}
      <div className="theme-surface w-[450px] flex flex-col bg-slate border-l border-overlay/5 shadow-2xl z-20">
        <div className="p-5 border-b border-overlay/5 bg-ink/20">
          <h3 className="font-bold text-mist flex items-center gap-2">
          ...
          <form onSubmit={handleSend} className="space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <select
                  required
                  className="w-full appearance-none bg-midnight-plum border border-overlay/10 rounded-lg px-3 py-2.5 text-sm text-mist focus:border-royal-amethyst focus:ring-1 focus:ring-royal-amethyst/50 outline-none transition-all cursor-pointer"
            ...
            <div className="space-y-1.5">
              ...
              <input
                required
                type="email"
                className={`w-full bg-midnight-plum border border-overlay/10 rounded-lg px-3 py-2.5 text-sm text-mist focus:border-royal-amethyst focus:ring-1 focus:ring-royal-amethyst/50 outline-none transition-all
            ...
            <div className="space-y-1.5">
              ...
              <input
                required
                className="w-full bg-midnight-plum border border-overlay/10 rounded-lg px-3 py-2.5 text-sm text-mist focus:border-royal-amethyst focus:ring-1 focus:ring-royal-amethyst/50 outline-none transition-all font-medium"
              ...
              <div className="py-2">
                {!aiDrafts ? (
                  <button
                    ...
                    disabled={generating}
                    className="group w-full py-3 border border-dashed border-overlay/10 rounded-xl text-soft-violet text-xs font-medium hover:bg-overlay/5 hover:border-royal-amethyst/50 hover:text-mist transition-all flex items-center justify-center gap-2"
                  >
                  ...
                  <div className="flex gap-2 p-1 bg-overlay/5 rounded-lg">
                    {["A", "B", "C"].map(k => (
                      <button
                        key={k}
                        type="button"
                        onClick={() => setSelectedDraft(k)}
                        className={`flex-1 py-1.5 text-xs rounded-md border font-medium transition-all ${selectedDraft === k ? "bg-royal-amethyst border-royal-amethyst text-white shadow" : "border-transparent text-slate-400 hover:text-mist hover:bg-overlay/5"}`}
                      >
                        Draft {k}
                      </button>
                    ))}
            ...
            <div className="space-y-1.5 flex-1">
              ...
              <textarea
                required
                className="w-full h-80 bg-midnight-plum border border-overlay/10 rounded-lg px-4 py-3 text-sm text-mist focus:border-royal-amethyst focus:ring-1 focus:ring-royal-amethyst/50 outline-none resize-none font-mono leading-relaxed custom-scrollbar"
